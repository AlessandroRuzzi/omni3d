...
#for id_data,dl in enumerate([(test_dl,"Test")]):
    ...
    for i, data in tqdm(enumerate(dl[0]), total=len(dl[0])):
        ...
        show_projection(torch.from_numpy(projector(np.array(verts))), cv2.imread(data["img_path"][0])[:,:,::-1].copy())
        ...
        y_min = min(object_center[1] - object_length[1]/2.0, human_center[1] - human_length[1]/2.0)
